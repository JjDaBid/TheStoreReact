/* eslint-disable react/prop-types */
const CardProduct = ({data}) => {
  return (
    <>      
        <div className='bg-white cursor-pointer w-60 h-64 rounded-lg transition duration-300 transform hover:scale-110 shadow-lg'>
              <figure className='relative mb-2 w-full h-4/5'>
                  <span className='absolute bottom-0 left-0 bg-white/60 rounded-lg text-black text-xs m-2 px-3 py-0.5'>{data.category}</span>
                  <img className='w-full h-full object-cover rounded-lg' src={data.image} alt={data.title} />
                  <div className='absolute top-0 right-0 flex justify-center items-center bg-white w-6 h-6 rounded-full m-2 p-1'>+</div>
              </figure>
              <p className='flex justify-between px-2 gap-1'>
                  <span className='text-sm font-light'>{data.title}</span>
                  <span className='text-lg font-medium'>${data.price.toLocaleString()}</span>
              </p>
        </div>
      
    </>
  )
}
export default CardProduct
